...
CSV_PATH = ROOT / "nsc_pair_table.csv"
JSON_PATH = ROOT / "nsc_certificates.json"
...
def nsc_pair_records(name: str, family: str, L: list[list[F]], B: list[list[F]]) -> dict:
    """Certify an instance and compute every NSC pair at every theta-half argmin."""
    P = P_of(L, B)
    delta = delta_of(P)
    assert 0 < delta <= F(1, 4), (name, delta)
    charts = chart_data(L, P)
    argmins = theta_argmins(charts)
    assert argmins, name
    pairs = []
    ...
    for cd in argmins:
        ...
        for s in pivots:
            for r in range(3):
                if r == s:
                    continue
                bmass = B_mass(P, cd.coords, cd.U, r, s)
                carrier_sum = F(0)
                carriers = []
                for i, a in enumerate(cd.coords):
                    beta = P[cd.U[r]][i]
                    if beta > 0 and a[s] < 0:
                        nu = row_neg(P[i])
                        b_contrib = beta * neg(a[s])
                        weighted_nu = beta * nu
                        carrier_sum += weighted_nu
                        carriers.append(
                            {
                                "i": i,
                                "a": a,
                                ...
                                "nu": nu,
                                "B_contrib": b_contrib,
                                "weighted_nu": weighted_nu,
                                "volume_inadmissible": neg(a[s]) * cd.m < F(1, 2),
                            }
                        )
                ratio = None if carrier_sum == 0 else bmass / carrier_sum
                rec = {
                    "name": name,
                    "family": family,
                    "n": len(L),
                    ...
                    "r": r,
                    "B": bmass,
                    "SUM_carriers": carrier_sum,
                    "R": ratio,
                    "carriers": carriers,
                }
                pairs.append(rec)
                if bmass > 0 and carrier_sum == 0:
                    zero_den_refuters.append(rec)
                if ratio is not None and bmass > 0:
                    if best_finite is None or ratio > best_finite["R"]:
                        best_finite = rec
    return {
        "name": name,
        "family": family,
        ...
        "argmins": argmins,
        "pairs": pairs,
        "zero_den_refuters": zero_den_refuters,
        "best_finite": best_finite,
    }


def known_nsc_instances() -> list[tuple[str, str, list[list[F]], list[list[F]]]]:
    g12_L = [
        [F(1), F(0), F(0)],
        [F(0), F(1), F(0)],
        [F(0), F(0), F(1)],
    ...
    insert_L, insert_B = family_compensated_insert(BASE_A, ORIGINAL_INSERT_H, (F(0), F(0), F(681, 10000)))
    two_L, two_B = family_two_carrier((F(3, 25), F(9, 100), F(1, 25), F(3, 100), F(33, 50), F(11, 50)))
    best_L, best_B = family_compensated_insert(BASE_A, CF_BEST_H, (F(0), F(0), CF_BEST_Y))
    return [
        ("G12-calibration", "known", g12_L, g12_B),
        ("insert-y=681/10000", "known", insert_L, insert_B),
        ("two-carrier-B", "known", two_L, two_B),
        ("amplifier-family-balance-rational", "amplifier-family", best_L, best_B),
    ]


def refuter_base_instance() -> tuple[list[list[F]], list[list[F]], tuple[F, F, F, F, F, F]]:
    # Found by a direct-R search in the two-carrier template.  The tuning
    # makes row c1 entrywise nonnegative while it remains an NSC carrier in
    # the pivoted argmin chart U=(c0,j,k).
    params = (F(1, 200), F(1, 100), F(0), F(1, 200), F(20099999, 20200500), F(1, 1000))
    L, B = family_two_carrier(params)
    return L, B, params


def clone_split_column(
    L: list[list[F]],
    B: list[list[F]],
    row_index: int,
    total_copies: int,
    ...
    out_L = [row[:] for row in L]
    out_B = [row[:] for row in B]
    for _ in range(total_copies - 1):
        out_L.append(L[row_index][:])
    for brow in out_B:
        old = brow[row_index]
        brow[row_index] = old / total_copies
        for _ in range(total_copies - 1):
            brow.append(old / total_copies)
    return out_L, out_B


def assert_known_ratios(certs: list[dict]) -> None:
    expected = {
        ("G12-calibration", (0, 1, 2), 2, 1): F(9, 4),
        ("insert-y=681/10000", (0, 2, 4), 2, 1): F(200000000, 175088281),
        ("two-carrier-B", (0, 2, 4), 2, 1): F(50000, 17919),
    }
    for cert in certs:
        for pair in cert["pairs"]:
            key = (pair["name"], pair["U"], pair["s"], pair["r"])
            if key in expected:
                assert pair["R"] == expected[key], (key, pair["R"], expected[key])
    seen = {key for cert in certs for pair in cert["pairs"] for key in [(pair["name"], pair["U"], pair["s"], pair["r"])] if key in expected}
    assert seen == set(expected), seen


def assert_refuter(cert: dict, expected_carriers: int) -> dict:
    matches = [
        p
        for p in cert["zero_den_refuters"]
        if p["U"] == (0, 3, 4) and p["s"] == 0 and p["r"] == 1
    ]
    assert len(matches) == 1, cert["name"]
    ref = matches[0]
    assert ref["B"] > 0
    assert ref["SUM_carriers"] == 0
    assert len(ref["carriers"]) == expected_carriers
    assert all(c["nu"] == 0 for c in ref["carriers"])
    assert all(c["volume_inadmissible"] for c in ref["carriers"])
    argmins = [cd.U for cd in cert["argmins"]]
    assert argmins == [(0, 3, 4)], (cert["name"], argmins)
    cd = cert["argmins"][0]
    assert cd.m == 1
    assert cd.phi == (0, 0, 0)
    return ref
...
def amplifier_family_R(a: F) -> F:
    # Along h=(-a,1/2+a,1/2), y=2679363/(49000*(22a+799)).
    y = active_boundary_y_for_shape_a(a)
    carrier_nu = F(3, 100) + (F(49, 50) * y * (a + F(1, 2)) - F(1071, 100000))
    return F(12, 197) / carrier_nu


def write_nsc_outputs(certs: list[dict], refuter: dict, clone_refuter: dict, hard_asserts: list[str]) -> None:
    all_pairs = [pair for cert in certs for pair in cert["pairs"]]
    finite_pairs = [p for p in all_pairs if p["R"] is not None and p["B"] > 0]
    finite_pairs.sort(key=lambda p: p["R"], reverse=True)
    zero_pairs = [p for p in all_pairs if p["B"] > 0 and p["SUM_carriers"] == 0]

    with CSV_PATH.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["name", "family", "n", "delta", "U", "m", "s", "r", "B", "SUM_carriers", "R", "carriers"])
        for p in all_pairs:
            w.writerow(
                [
                    p["name"],
                    ...
                    p["n"],
                    fstr(p["delta"]),
                    " ".join(str(x) for x in p["U"]),
                    fstr(p["m"]),
                    p["s"],
                    p["r"],
                    fstr(p["B"]),
                    fstr(p["SUM_carriers"]),
                    "INF" if p["B"] > 0 and p["SUM_carriers"] == 0 else (fstr(p["R"]) if p["R"] is not None else ""),
                    "; ".join(
                        f"i={c['i']}:beta={fstr(c['beta'])},a_s={fstr(c['a_s'])},nu={fstr(c['nu'])},inad={c['volume_inadmissible']}"
                        for c in p["carriers"]
                    ),
                ]
            )
    ...
    payload = {
        "status": "T0 exact Fraction certificates for reported matrices; L3 search evidence otherwise",
        ...
        "certificates": [
            {
                ...
                "B_left": matrix_to_json(c["B_left"]),
                "P": matrix_to_json(c["P"]),
                "charts": [
                    {
                        "U": list(cd.U),
                        "volume": fstr(cd.volume),
                        "m": fstr(cd.m),
                        "phi": [fstr(x) for x in cd.phi],
                        "Phi": fstr(cd.Phi),
                        "theta_half": cd.m >= F(1, 2),
                        "argmin": cd in c["argmins"],
                    }
                    for cd in c["charts"]
                ],
                "pairs": [
                    {
                        ...
                        "r": p["r"],
                        "B": fstr(p["B"]),
                        "SUM_carriers": fstr(p["SUM_carriers"]),
                        "R": None if p["R"] is None else fstr(p["R"]),
                        "zero_den_refuter": p["B"] > 0 and p["SUM_carriers"] == 0,
                        "carriers": [
                            {
                                "i": cc["i"],
                                "a": [fstr(x) for x in cc["a"]],
                                "beta": fstr(cc["beta"]),
                                "a_s": fstr(cc["a_s"]),
                                "nu": fstr(cc["nu"]),
                                "B_contrib": fstr(cc["B_contrib"]),
                                "weighted_nu": fstr(cc["weighted_nu"]),
                                "volume_inadmissible": cc["volume_inadmissible"],
                            }
                            for cc in p["carriers"]
                        ],
                    }
                    for p in c["pairs"]
                ],
    ...
    ref_cert = next(c for c in certs if c["name"] == "direct-zero-den-refuter")
    clone_cert = next(c for c in certs if c["name"] == "three-carrier-zero-den-refuter")
    best_finite = finite_pairs[0]
    known_rows = [
        p
        for p in all_pairs
        if (p["name"], p["U"], p["s"], p["r"])
        in {
            ("G12-calibration", (0, 1, 2), 2, 1),
            ("insert-y=681/10000", (0, 2, 4), 2, 1),
            ("two-carrier-B", (0, 2, 4), 2, 1),
        }
    ]
    known_rows.sort(key=lambda p: p["name"])

    lines: list[str] = []
    lines.append("# DC2 NSC-Ratio Direct Search Report")
    lines.append("")
    lines.append("## Verdict")
    lines.append("")
    lines.append(
        "[T0] The broad `conj-nsc` contract is refuted by an exact rank-3 signed idempotent with "
        "`B_{1,0}=1/4020000000 > 0` and `SUM_carriers=0` at the capped theta-half Phi-argmin "
        "`U=(0,3,4)`, maximal pivot `s=0`, transverse `r=1`."
    )
    lines.append("")
    lines.append(
        "[T0] The carrier row is entrywise nonnegative in the ambient idempotent, so its "
        "`nu_i(P)` is exactly zero. This is the prompt's zero-denominator refutation mode, not merely "
        "a large finite lower bound for `K0`."
    )
    lines.append("")
    lines.append("## Re-run")
    lines.append("")
    lines.append("```bash")
    lines.append("python3 waves-scratch/dc2-nsc-ratio/nsc_ratio_search.py")
    lines.append("```")
    lines.append("")
    lines.append("## Refuting Certificate")
    lines.append("")
    lines.append("Two-carrier parameters `(p,e,q,g,v,w)`:")
    lines.append("")
    lines.append("```text")
    lines.append("(1/200, 1/100, 0, 1/200, 20099999/20200500, 1/1000)")
    lines.append("```")
    lines.append("")
    lines.append("`L`:")
    lines.append("")
    lines.append("```text")
    for row in ref_cert["L"]:
        lines.append("[" + "  ".join(fstr(x) for x in row) + "]")
    lines.append("```")
    lines.append("")
    lines.append("`B_left`:")
    lines.append("")
    lines.append("```text")
    for row in ref_cert["B_left"]:
        lines.append("[" + "  ".join(fstr(x) for x in row) + "]")
    lines.append("```")
    lines.append("")
    lines.append("`P=L B_left` with row negative masses:")
    lines.append("")
    lines.append("```text")
    for idx, row in enumerate(ref_cert["P"]):
        lines.append("row " + str(idx) + ": [" + "  ".join(fstr(x) for x in row) + f"]   nu={fstr(row_neg(row))}")
    lines.append("```")
    lines.append("")
    lines.append(f"`delta(P)={fstr(ref_cert['delta'])}`.")
    lines.append("")
    lines.append("Complete chart enumeration:")
    lines.append("")
    lines.append("| chart | volume | m | Phi-vector | max Phi | theta-half | argmin |")
    lines.append("|---|---:|---:|---|---:|---:|---:|")
    for cd in ref_cert["charts"]:
        lines.append(
            f"| `{cd.U}` | `{fstr(cd.volume)}` | `{fstr(cd.m)}` | "
            f"`({', '.join(fstr(x) for x in cd.phi)})` | `{fstr(cd.Phi)}` | "
            f"{cd.m >= F(1, 2)} | {cd in ref_cert['argmins']} |"
        )
    lines.append("")
    lines.append("Refuting pair:")
    lines.append("")
    lines.append(
        f"- [T0] `U={refuter['U']}`, `m={fstr(refuter['m'])}`, `Phi=(0,0,0)`, "
        f"`s={refuter['s']}`, `r={refuter['r']}`."
    )
    lines.append(
        f"- [T0] `B_{{1,0}}={fstr(refuter['B'])}`, `SUM_carriers={fstr(refuter['SUM_carriers'])}`."
    )
    for c in refuter["carriers"]:
        lines.append(
            f"- [T0] carrier `i={c['i']}` has `a=({', '.join(fstr(x) for x in c['a'])})`, "
            f"`beta={fstr(c['beta'])}`, `a_s={fstr(c['a_s'])}`, `nu={fstr(c['nu'])}`, "
            f"`B_contrib={fstr(c['B_contrib'])}`, volume-inadmissible `{c['volume_inadmissible']}`."
        )
    lines.append("")
    lines.append("## Known Witness Reconstruction")
    lines.append("")
    lines.append("| instance | certified pair | B | SUM_carriers | R | carriers |")
    lines.append("|---|---|---:|---:|---:|---|")
    for p in known_rows:
        carriers = ", ".join(f"i={c['i']} nu={fstr(c['nu'])} inad={c['volume_inadmissible']}" for c in p["carriers"])
        lines.append(
            f"| `{p['name']}` | `U={p['U']}, s={p['s']}, r={p['r']}` | "
            f"`{fstr(p['B'])}` | `{fstr(p['SUM_carriers'])}` | `{fstr(p['R'])}` | {carriers} |"
        )
    lines.append("")
    lines.append("The script also recomputes every transverse pair at every maximal pivot; see `nsc_pair_table.csv`.")
    lines.append("")
    lines.append("## Search Coverage")
    lines.append("")
    lines.append(
        "- [T0/T1] Perturb-known/two-carrier search: lowering the carrier's own ambient negativity in the "
        "two-carrier template led to the displayed zero-denominator matrix. The key design choice was to "
        "pivot to `U=(c0,j,k)` and make the outside row `c1` an NSC carrier while keeping row `c1` "
        "entrywise nonnegative."
    )
    lines.append(
        "- [T0] Multi-carrier design: splitting the `c1` column into three identical actual rows gives "
        f"`B={fstr(clone_refuter['B'])}`, `SUM_carriers=0`, and three volume-inadmissible zero-nu carriers. "
        f"The cloned certificate has `n={clone_cert['n']}` and the same argmin `U=(0,3,4)`."
    )
    lines.append(
        "- [T1] Amplifier family law: along the recorded boundary family "
        "`h=(-a,1/2+a,1/2)`, `y=2679363/(49000*(22a+799))`, the single-carrier NSC ratio is"
    )
    lines.append("")
    lines.append("```text")
    lines.append("R_amp(a) = (12/197) / (1929/100000 + (49/50)*(a+1/2)*2679363/(49000*(22a+799))).")
    lines.append("```")
    lines.append("")
    lines.append(
        f"At the rational balance checkpoint `a=6332623/370881409`, "
        f"`R_amp={fstr(amplifier_family_R(CF_BEST_A))}` (~{float(amplifier_family_R(CF_BEST_A)):.6f}); "
        "this family is not an NSC-ratio stress compared with the zero-denominator refuter."
    )
    lines.append(
        "- [T2] Additional richer shapes were not needed for refutation; the banked script includes the "
        "three-carrier split as a non-amplifying multi-carrier check. It preserves the same obstruction "
        "instead of producing a finite large ratio."
    )
    lines.append("")
    lines.append("## Supremum Table")
    lines.append("")
    lines.append("| class | result |")
    lines.append("|---|---|")
    lines.append(f"| finite certified pairs scanned | best finite `R={fstr(best_finite['R'])}` at `{best_finite['name']}`, `U={best_finite['U']}`, `s={best_finite['s']}`, `r={best_finite['r']}` |")
    lines.append(f"| zero-denominator certified pairs | `{len(zero_pairs)}` pair(s), including the direct refuter and three-carrier split |")
    lines.append("| conjecture status | refuted in the broad contract; no finite `K0` can repair `B>0 <= K0*0` |")
    lines.append("")
    lines.append("## Hard Asserts")
    lines.append("")
    for item in hard_asserts:
        lines.append(f"- [T0] {item}")
    lines.append("")
    lines.append("## Tier Discipline")
    lines.append("")
    lines.append("- [T0] Exact matrix identities, chart enumerations, ratios, and zero-denominator claims are `fractions.Fraction` certificates.")
    lines.append("- [T1] The displayed amplifier-family formula is algebra from the repo's recorded family law plus exact sign pattern checks at the certified checkpoints.")
    lines.append("- [T2] Search coverage is finite and template-biased; it is not an exhaustive rank-3 classification.")
    lines.append("- [T3] No speculative theorem is used in the verdict.")
    ANSWER_PATH.write_text("\n".join(lines) + "\n")


def main() -> None:
    certs: list[dict] = []
    for name, family, L, B in known_nsc_instances():
        certs.append(nsc_pair_records(name, family, L, B))

    ref_L, ref_B, _params = refuter_base_instance()
    ref_cert = nsc_pair_records("direct-zero-den-refuter", "direct-R-two-carrier", ref_L, ref_B)
    certs.append(ref_cert)

    clone_L, clone_B = clone_split_column(ref_L, ref_B, row_index=1, total_copies=3)
    clone_cert = nsc_pair_records("three-carrier-zero-den-refuter", "multi-carrier-clone-split", clone_L, clone_B)
    certs.append(clone_cert)

    assert_known_ratios(certs)
    refuter = assert_refuter(ref_cert, expected_carriers=1)
    clone_refuter = assert_refuter(clone_cert, expected_carriers=3)

    # Exact checkpoints for the amplifier-family R law.
    amp_cert = next(c for c in certs if c["name"] == "amplifier-family-balance-rational")
    amp_pair = next(p for p in amp_cert["pairs"] if p["U"] == (0, 2, 4) and p["s"] == 2 and p["r"] == 1)
    assert amp_pair["R"] == amplifier_family_R(CF_BEST_A)

    hard_asserts = [
        "For every certified instance, `B_left * L = I_3`, `P=L*B_left`, `P^2=P`, and every row sum is `1`.",
        "For every certified instance, `0 < delta(P) <= 1/4`.",
        "For every certified instance, all actual-row charts are enumerated and theta-half argmins are selected by exact `Phi` comparison.",
        "Known ratios are reconstructed exactly: `200000000/175088281`, `50000/17919`, and `9/4`.",
        "Direct refuter has unique theta-half argmin `U=(0,3,4)` with `m=1` and `phi=(0,0,0)`.",
        "Direct refuter pair `(s,r)=(0,1)` has `B=1/4020000000`, `SUM_carriers=0`, and one carrier with `nu=0`.",
        "Three-carrier split preserves the same zero-denominator refutation with three zero-nu volume-inadmissible carriers.",
        "Amplifier-family rational checkpoint satisfies the displayed `R_amp(a)` law exactly.",
    ]
    write_nsc_outputs(certs, refuter, clone_refuter, hard_asserts)
