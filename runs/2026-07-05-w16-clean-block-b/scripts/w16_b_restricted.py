#!/usr/bin/env python3
"""Exact-rational wave-16 branch-restricted B-lemma amplifier worker.
...
def write_outputs(points: list[dict], obstructions: list[dict], calibrations: dict) -> None:
    ...
    lines: list[str] = []
    lines.append("# Wave 16 branch-restricted B-lemma amplifier report")
    lines.append("")
    lines.append("Status: T0 exact certificates for the listed instances; L3 numerical evidence only, not a proof.")
    lines.append("")
    lines.append("## Headline")
    lines.append("")
    lines.append(
        f"Best certified clean Gamma-block argmin found: `{fstr(best['B_over_delta'])}` "
        f"at `delta={fstr(best['delta'])}` (`{best['name']}`, n={best['n']})."
    )
    lines.append("")
    lines.append(
        f"This beats the wave-15 seed `8400000/10897843` and crosses the rounded `0.771` level, "
    ...
    lines.append("")
    lines.append("- (i) Beat the wave-15 seed / rounded `0.771`: YES.")
