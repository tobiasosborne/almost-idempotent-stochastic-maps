...
kind: lemma
contract: For every putative SL1a counterexample datum (P,v,lambda), every top support functional phi at v, and every admissible exposer h at v, some row point f in supp(lambda) satisfies 2(H-phi(p_f))/(2+4delta(P)) + h(p_f) <= 12sqrt(delta(P))/13.
