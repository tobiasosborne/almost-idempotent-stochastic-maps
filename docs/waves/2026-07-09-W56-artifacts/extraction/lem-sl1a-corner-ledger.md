...
kind: lemma
contract: For every exact signed idempotent P with 0 < delta(P) <= 2^(-16), every SL1a top datum (v,phi,h), every far co-top row point f with 2(H-phi(p_f))/(2+4delta(P)) + h(p_f) <= 12sqrt(delta(P))/13, and every legal vertex kernel xi, the coupled measure Gamma_f(x,u) = P_fx^+ xi_x(u) assigns at least 58079731/109051904 > 1/2 mass to the corner where H-phi and h are both < 4sqrt(delta(P)) on both coordinates, every point of that corner has both coordinate depths > H-4sqrt(delta(P)), and its vertex coordinate is hidden.
