<!--
WAVE: arm G, wave 13 (amplifier branch) — 2026-07-04, session 7
WORKER: fresh codex exec (prompt: session scratchpad waves-scratch/PROMPT-wave13-amplifier.md)
ORCHESTRATOR: script re-run clean (exit 0); record instance independently recomputed (P^2=P, delta,
  the (0,1,3)/(0,2,4) argmin TIE, B=42/985, B/delta = 90516217933510287011133600/116398566910735274162898787
  ~ 0.77764) — all match. Worker ANSWER below VERBATIM. Data banked: runs/2026-07-04-b-amplifier-hunt/.
ORCHESTRATOR CORRECTION (honesty): the prompt's target (iii) ("B > Phi_r(U)+I kills the skeleton") was
  MIS-SPECIFIED — the skeleton bounds I BY B (import reduction), so B exceeding Phi_r+I does not break
  the chain. The informative content of the worker's (iii)=YES is: B is NOT covered by the existing
  pivot-s budget terms (B/budget ~ 4.24 at the record point, extending the kill-test's 1.82) — i.e. the
  B-term needs its OWN delta-scale financing (the NSC / aism-z98 shape), which was already the wave-13
  premise. No skeleton kill occurred.
-->
