# DESIGN — MAIN-CB two-defect interface repair (v2)

**Date:** 2026-08-01  
**Status:** DESIGN ONLY; hostile-audited (AUDIT-MAINCB-REPAIR.md: DESIGN-CONFIRMED after F1–F3); awaiting user ratification  
**v2 note:** this file is v1 (`DESIGN-MAINCB-REPAIR.md`, kept intact for the audit's line citations) with the auditor's three mandatory exact corrections F1–F3 applied VERBATIM (`AUDIT-MAINCB-REPAIR.md` §1): F1 re-derives `lem-maincb-isomorphism-unit-control` as a modular export of the existing unit clause (`C_iso_unit=1`); F2 fixes the ill-typed `m=1` codomain identification in M19-S1 (+1 dep, budget 10→11, +1 re-seed node); F3 replaces M20's opaque `e_sim,e_full` binder by explicit provider-first fixing. §10's characterization of the first bridge is to be read subject to F1, and F2 is the Stage-1 codomain risk (AUDIT §4). No other content differs from v1.  
...
| (new) m=1 ambient transfer | One new node (AUDIT F2): compose the supplied fresh `C^2->A_fresh=S_{P_fresh}` inclusion with the canonical amplified linear embedding `A_fresh->A`, using `lem-compcb-rectangular-product`, the compressed-unit helper, `P_fresh=w(I_C)`, and the incoming global unit estimate to obtain the `A`-valued inclusion and unit bounds. |

Expected new build: about 11 live nodes, three rounds, cap 14. The red test is to remove the inequalities relating `W.c0_cb`, `W.K1`, or `W.e1` to their providers: the binder node must fail.
