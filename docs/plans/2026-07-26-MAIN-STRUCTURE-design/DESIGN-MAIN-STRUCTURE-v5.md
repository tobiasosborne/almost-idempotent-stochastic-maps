# DESIGN-MAIN-STRUCTURE-v5 — prescribed two-defect repair

**Date:** 2026-07-27
**Role:** fresh independent fifth-repair designer
...
## 0. Verdict and exact delta from v4

**Verdict: DESIGNED-CLOSABLE, conditional on two explicit serial gates.**
This is the prescribed narrow repair of
`DESIGN-MAIN-STRUCTURE-v4.md` under the binding
`AUDIT-MAIN-STRUCTURE-v4.md`. It does not redesign any audit-confirmed
...
The exact changes from v4, and only those changes, are:

| v5 change | binding cause |
|---|---|
| Tie M19-S2 and M19-S3's supplied partition state to the displayed \(A,w\), and tie their supplied reset state(s) to the \(U\), respectively \(U,V\), of that same partition state. | Audit §3.1. |
| Add `lem-compcb-corner-algebra` as a direct M13 dependency, make it produce the extended-algebra field of \(A_R\), and absorb its \(e_{\rm ca}\) threshold and \(C_{\rm ca}\) coefficient into \(e_{\rm s2}\) and \(C_{\rm s2}\). | Audit §3.2. |

Every row and mechanism that `AUDIT-MAIN-STRUCTURE-v4.md` marked **VALID**
or exact-retained is copied without mathematical change. The v3-audit
defects A, C, and D, the M07-import defect, P0, M03/M04/M11/M12/M19-S1/
M19-R, M20, M25-M27, M28's target contract, the M28-only join, M27's
complete-family hypothesis, M16's \(C_{\rm s2}\) absorption, R19/R21, and
G-S1's placement remain unchanged. The landing order, escalation ledger,
and §12 disposition change only to record the two repairs above.
...
| **M13** `lem-maincb-stage2-extcb-datum` | There are universal \(C_{\rm s2}\ge1\) and \(e_{\rm s2}>0\), with the \(e_{\rm ca}\) threshold and universal \(C_{\rm ca}\) coefficient of `lem-compcb-corner-algebra` absorbed into \(e_{\rm s2}\) and \(C_{\rm s2}\), such that, if \(A\) is a finite-dimensional extended \(\varepsilon_A\)-\(C^*\)-algebra with \(\varepsilon_A\le t\), a supplied MAIN partition state comes from a non-unital extended \(t\)-inclusion \(w:\mathbb C^m\to A\) with one-dimensional images \(P_j\), has nonempty \(U\), \(j\notin U\), \(\dim S^A_{P_k,P_j}=1\) for every \(k\in U\), and \(R=U\cup\{j\}\), and a supplied current reset state \(v_U:M_{|U|}\to A_U\) is an extended isomorphism satisfying \(\varepsilon_U,d_U\le t\le e_{\rm s2}\) and \(d_U\le c_0^{\rm cb}\varepsilon_U\), then `lem-compcb-corner-algebra` makes \(A_R\) an extended \(\varepsilon_{A_R}\)-\(C^*\)-algebra, M07 makes \(P_U^R,P_j^R\) quantitative projections in \(A_R\), and together with the M09 outer-compressed isomorphism they satisfy every `def-extcb-datum` clause—approximate complementarity to \(I_{A_R}\), one-dimensional \(S_{P_j^R}\), nonzero \(S_{P_U^R,P_j^R}\), and total error \(e=\delta+\varepsilon_{A_R}\)—with \(e\le C_{\rm s2}t\), forming the explicit Stage-2 raw-call closed EXT-CB datum in \(A_R\). | `def-maincb-partition-state`; `def-maincb-reset-state`; `def-maincb-raw-call`; `def-extcb-datum` | `lem-maincb-error-improvement`; `lem-maincb-nested-corner-comparison`; `lem-maincb-nested-corner-dimension-transport`; `lem-maincb-outer-compression-transfer`; `lem-maincb-corner-equivalence`; `lem-compcb-corner-algebra`; `lem-extcb-corner-dimension-additivity` | `approximate_algebras.tex:1363-1412,1430-1441`; `definitions/def-extcb-datum.md:13-17` | 10 / 3 | **SUPPORTED-WITH-DERIVATION.** Audit-v3 correction restores the ambient/M07 inputs and makes datum closure explicit; audit-v4 §3.2 adds the direct corner-algebra producer and absorbs its threshold/coefficient into \(e_{\rm s2},C_{\rm s2}\). |
...
The corner-algebra guard is absorbed into \(e_{\rm s2}\), its universal
coefficient is absorbed into \(C_{\rm s2}\), and the output is
\(C_{\rm s2}t\), not \(t\).
...
4. Land M10, then corrected M11. Land M12 and M13 only with their explicit
   original-ambient hypotheses and direct M07 imports; M13 additionally
   imports `lem-compcb-corner-algebra` directly and absorbs its
   \(e_{\rm ca},C_{\rm ca}\) data into \(e_{\rm s2},C_{\rm s2}\).
...
8. Land corrected M19-S1, M19-S2, M19-S3, then corrected M19-R. For S2/S3,
   validate that the partition state records the displayed \(A,w\) and that
   the reset state(s) refer to its \(U\), respectively \(U,V\).
...
- **INTERFACE ESCALATED:** partition state does not store the original
  ...
  Each consumer must identify the state's recorded ambient and map with
  those quantified \(A,w\). Simultaneous \(U,V\) use requires two supplied
  reset states referring to the \(U,V\) of that same partition state.
...
- **INTERFACE/DEPENDENCY ESCALATED:** M11-M13 carry the original
  ...
  additionally imports `lem-compcb-corner-algebra` directly and uses it to
  fill the extended-algebra field of \(A_R\), so every closed-EXT datum field
  has an exact producer.
- **SCALE ESCALATED:** M13 has \(C_{\rm s2}\ge1\); M16 accepts total datum
  ...
  M13's \(e_{\rm s2}\) also absorbs the corner-algebra \(e_{\rm ca}\)
  threshold and \(C_{\rm s2}\) absorbs its universal \(C_{\rm ca}\)
  coefficient.
...
- **DOMAIN ESCALATED:** M19-S1 restores the unit-preserving v2 inclusion;
  ...
  ambient and extended \(c_0^{\rm cb}\varepsilon\)-inclusion \(w\). In
  M19-S2/S3 the supplied partition state is for this same \(A,w\), and the
  supplied reset state(s) refer to its \(U\), respectively \(U,V\).
...
## 12. Disposition of every `AUDIT-MAIN-STRUCTURE-v4.md` finding

“CLEARED-BY” means repaired at design-architecture level only. It never means
proved or authorized to land. “UNCHANGED-VALID” means the v4 row,
architecture, source check, or hazard finding is copied without mathematical
change. Audit loci below refer to `AUDIT-MAIN-STRUCTURE-v4.md`.

### 12.1 Two decisive defects and the binding v3-audit verdicts

| binding v4-audit finding | v5 disposition |
|---|---|
| M19-S2/S3 quantify good \(A,w\) independently of the partition state whose corners enter their conclusions (§1 item 1; §3.1). | **CLEARED-BY M19-S2/S3:** each now says “a supplied MAIN partition state for this same \(A,w\),” and each supplied reset state is tied to the \(U\), respectively \(U,V\), of that same state. M04 therefore bounds the displayed corners. |
| M13 has no exact producer for the extended-algebra field of \(A_R\) (§1 item 2; §3.2). | **CLEARED-BY M13:** `lem-compcb-corner-algebra` is a direct dependency, produces the extended \(A_R\) field, and has its \(e_{\rm ca}\) threshold and \(C_{\rm ca}\) coefficient absorbed into \(e_{\rm s2},C_{\rm s2}\). |
| V3 defect A — M19-S1's non-unital domain (§2). | **UNCHANGED-VALID:** M19-S1 retains the finite-dimensional extended ambient and extended inclusion including its unit clause. |
| V3 defect B — closed M19-S2/S3 and structural domains (§2). | **CLEARED-BY M19-S2/S3:** v4's quantitative domains are retained and the missing identity tie is restored. M25-M27 retain their already-valid “comes from” ties. |
| V3 defect C — M19-R's M03 eligibility (§2). | **UNCHANGED-VALID:** M19-R retains the actual extended raw inclusion/isomorphism hypothesis from its named finite-dimensional source. |
| V3 defect D — compressed-corner scalar call (§2). | **UNCHANGED-VALID:** the distinct scalar tag, \(t_{\rm atom}=K_{\rm call}\varepsilon\), and M04 inequalities are retained. |
| V3 exact-dependency defect — direct M07 in M12/M13 (§2). | **UNCHANGED-VALID:** both direct M07 imports are retained; M13's new corner-algebra import is a separate v4-audit repair. |
| P0 schema correction (§2; §6). | **UNCHANGED-VALID AT DESIGN-PROPOSAL LEVEL:** all four proposals, the consumer-quantified bounds, two-reset-state rule, byte-verbatim operator-space body, and hard stop are retained. |
| V3 per-row correction package was incomplete only at M13 and M19-S2/S3 (§2). | **CLEARED-BY the two prescribed v5 repairs;** every other corrected row remains unchanged-valid. |

### 12.2 Every v4-audited changed row

| audited row | v5 disposition |
|---|---|
| M03 | **UNCHANGED-VALID:** the landed contract remains character-for-character exact and only its proposed dependency is M02. |
| M04 | **UNCHANGED-VALID:** the finite-dimensional extended ambient remains explicit. |
| M11 | **UNCHANGED-VALID:** the original ambient bound remains explicit. |
| M12 | **UNCHANGED-VALID:** the ambient bound, two reset states, and direct M07 import remain explicit. |
| M13 | **CLEARED-BY direct `lem-compcb-corner-algebra`:** the target remains coherent and now every `def-extcb-datum` field has an exact producer. |
| M19-S1 | **UNCHANGED-VALID, BLOCKED ON G-S1:** the unit clause and hard gate are retained. |
| M19-S2 | **CLEARED-BY the same-\(A,w\) partition-state clause and the same-state-\(U\) reset clause.** |
| M19-S3 | **CLEARED-BY the same-\(A,w\) partition-state clause and the same-state-\(U,V\) reset clauses.** |
| M19-R | **UNCHANGED-VALID:** its actual-map, finite-source, and extended-ambient hypotheses still match M03. |
| M20 | **UNCHANGED-VALID WITH CLEARED PARENTS:** its finite-minimum and \(t_{\rm atom}\) arithmetic are unchanged; S2/S3 are now closed. |
| M25 | **UNCHANGED-VALID WITH CLEARED PARENTS:** its global domain, direct M04 import, atomic base scale, and local invariant are unchanged. |
| M26 | **UNCHANGED-VALID WITH CLEARED PARENT:** its own state remains tied to the restored global \(A,w\); M19-S3 is now closed. |
| M27 | **UNCHANGED-VALID WITH CLEARED PARENT:** its complete-family hypothesis and M26-only edge remain unchanged. |

### 12.3 Every exact-retained row

| retained row | v5 disposition |
|---|---|
| M01 | **UNCHANGED-VALID, gated on P0.** |
| M02 | **UNCHANGED-VALID, gated on P0/M01.** |
| M05 | **UNCHANGED-VALID.** |
| M06 | **UNCHANGED-VALID.** |
| M07 | **UNCHANGED-VALID** with the fixed-telescope gap discipline retained. |
| M08 | **UNCHANGED-VALID.** |
| M09 | **UNCHANGED-VALID.** |
| M10 | **UNCHANGED-VALID.** |
| M14 | **UNCHANGED-VALID, gated on operator-space P0.** |
| M15 | **UNCHANGED-VALID.** |
| M16 | **UNCHANGED-VALID:** it still absorbs \(C_{\rm s2}\) into \(D_2,e_2\); M13 now also absorbs the corner-algebra coefficient into \(C_{\rm s2}\). |
| M17 | **UNCHANGED-VALID.** |
| M18 | **UNCHANGED-VALID.** |
| M21 | **UNCHANGED-VALID WITH CLEARED PARENTS.** |
| M22 | **UNCHANGED-VALID WITH CLEARED PARENTS.** |
| M23 | **UNCHANGED-VALID, blocked on G-S1.** |
| M24 | **UNCHANGED-VALID, blocked on G-S1/M23.** |
| M28 | **UNCHANGED-VALID WITH CLEARED SUBTREE:** its target contract and M28-only join remain unchanged. |

No silent retained-row drift is introduced. In particular M27's
complete-family input and the M28-only join remain exactly as audited in v4.

### 12.4 Eight requirements, source checks, landing, hazards, and scope

| v4-audit finding | v5 disposition |
|---|---|
| P0 before M01 (§6). | **UNCHANGED-VALID:** the hard stop and topological step 0 remain. |
| Closed call-specific envelopes (§6). | **CLEARED-BY M19-S2/S3's identity ties:** the displayed \(A,w\), partition state, and reset corners now form one datum. |
| \(C_{\rm s2}\) option (§6). | **UNCHANGED-VALID:** M16's absorption is retained; M13's expanded \(C_{\rm s2}\) remains universal. |
| Strong reset invariant (§6). | **UNCHANGED-VALID:** M19-R still derives it only from an eligible actual raw map. |
| Exact dependencies (§6). | **CLEARED-BY M13's direct `lem-compcb-corner-algebra` import;** all previously audited exact imports remain unchanged-valid. |
| G-S1 hard stop (§6). | **UNCHANGED-VALID:** it remains after M18 and before every M19/structural row. |
| M27 complete family / M28-only join (§6). | **UNCHANGED-VALID.** |
| Complete escalation ledger (§6). | **CLEARED-BY §§10-11:** the same-\(A,w\)/same-state reset identities and M13's corner-algebra dependency, threshold, and coefficient are all recorded. |
| P0 frontmatter and generated-index absence (§6). | **UNCHANGED-VALID:** the proposals remain schema-complete but unlanded; P0 remains a genuine stop. |
| Operator-space body byte-equals TeX 1453-1464 and has the audited SHA prefix (§6). | **UNCHANGED-VALID:** the cited block and provenance are copied byte-stable. |
| Landing step 4 lacked M13's ambient producer; step 8 would land underbound S2/S3 (§7). | **CLEARED-BY §10 steps 4 and 8:** the direct producer/threshold absorption and the identity checks are explicit serial annotations. |
| R19 maximal selection, R21's two inductions, and R22's two-zero-corner mechanism (§7). | **UNCHANGED-VALID.** |
| Dimension-freeness of the new scalar scale and all retained constants (§7). | **UNCHANGED-VALID:** v5 uses only the existing universal \(e_{\rm ca},C_{\rm ca}\) and identity constraints; no amplification, dimension, class-count, block, or induction-length dependence is introduced. |
| No route-level obstruction or counterexample to Kitaev was found (§1, §7). | **UNCHANGED-VALID:** both v5 changes are factoring/interface repairs. |
| No authorization, status promotion, or downstream mutation; `op-classical` remains open (§7). | **UNCHANGED-VALID:** this document is design-only and changes no registry, definition, dependency, or status. |
...
## 13. Historical disposition of every `AUDIT-MAIN-STRUCTURE-v3.md` finding
...
### 13.1 Fatal defects, dependency defect, and P0
...
### 13.2 Landed inputs and every pre-gate row
...
### 13.3 Every post-gate row, hazards, dimension, and retained contracts
...
### 13.4 Audit of every v3-disposition claim and execution ledger
...
## 14. Historical disposition of `AUDIT-MAIN-STRUCTURE-v2.md`
...
### 14.1 Fatal defects, landed inputs, and load-bearing comparisons
...
### 14.3 Hazards, G-S1, dimension, and inherited defects
...
### 14.4 Landing-order, ledger, and all eight §10 requirements
