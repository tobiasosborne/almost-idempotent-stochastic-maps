...
| 4 | The rigour ceiling is codex; L5 tower can rest on `stated` bridges | (a) Hostile pass on all stated bridges is MANDATORY before consumption (aism-bbv); (b) long-term: scope an af→Lean bridge for the ~12-node trunk so the ladder's top touches mathlib ground truth | (a) P0 queued; (b) P1 scoping bead |
