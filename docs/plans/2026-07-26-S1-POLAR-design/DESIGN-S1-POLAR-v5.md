# DESIGN — fifth repair of closed contracts for GAP-S1-POLAR-CONTRACT
...
Exactly six changes are made to v4; everything else is carried forward.

| v5 change | binding audit item forcing it |
|---|---|
| Repair \((A_3)\) by universally binding the family of unique \(C^1\) graph maps through only its defining zero equation, and keep only the tangent-image, Maurer–Cartan distortion, and two equivariance conclusions in the consequent. | `AUDIT-S1-POLAR-v4.md` §§1.3, 2.2, 6(2)–(3). |
| Repair \((A_5)\) (v4 label \((A_6)\)) by restoring the producer domain of every exact-unit algebra and affirmatively binding the unique polar inverse before defining the global \(C^1\) group maps; preserve all seven identities and estimates. | `AUDIT-S1-POLAR-v4.md` §§1.5, 2.2, 6(1)–(3). |
| Repair \((A_6)\) (v4 label \((A_7)\)) by restoring the producer domain of every exact-unit algebra and locally binding the unique first polar-inverse component used in the jointly continuous equivariant path. | `AUDIT-S1-POLAR-v4.md` §§1.6, 2.2, 6(1)–(3). |
| Repair \((A_7)\) (v4 label \((A_8)\)) by restoring the producer domain of every exact-unit algebra and explicitly defining the unique polar inverse, unique graph map, \(\chi_s\), and \(\sigma\), without conditionalizing them; retain only same-chart retention and the \(-2I\) derivative estimate as conclusions. | `AUDIT-S1-POLAR-v4.md` §§1.7, 2.2, 6(1)–(3). |
| Factor seven parameterized monotonicity/transport helpers, each receiving the same tuple \(W\), and replace row 13's direct producer imports by those helpers. Under the declared atomic-import convention, each helper is \(4/2\), while the common ledger is honestly \(11/3\); the unfactored projection remains \(18/4\) and is not used. | `AUDIT-S1-POLAR-v4.md` §§3, 6(4). |
| Replace the disposition section by an exhaustive table for every v4-audit finding; preserve all v4-VALID surfaces, including \((A_1),(A_2),(A_4),(R)\), rows 1–12, downstream rows and definitions, ledgers, dimension-freeness, serial dependencies, and sources. | `AUDIT-S1-POLAR-v4.md` §§1.1–1.8, 2.1, 4–6. |
...
## 10. Disposition of every `AUDIT-S1-POLAR-v4.md` finding
...
### 10.1 Final disposition and the eight analytic clauses

| audit-v4 finding | disposition |
|---|---|
| §0 overall `REDESIGN` | **CLEARED-BY** the four clause repairs and seven parameterized transport helpers described in §0; no route-level result is claimed. |
| §0 silent finite-dimensional weakening in the group, path, and derivative clauses | **CLEARED-BY** \((A_5),(A_6),(A_7)\), each of which now quantifies over every exact-unit \(\varepsilon_r\)-\(C^*\)-algebra. |
| §0 group and derivative conclusions were conditionalized | **CLEARED-BY** affirmative, locally bound unique polar-inverse/graph objects and affirmative map conclusions in \((A_5)\) and \((A_7)\). |
| §0 Maurer–Cartan and derivative maps were unbound; \(W\) cannot carry maps | **CLEARED-BY** explicit object-level map binders in \((A_3)\) and \((A_7)\); \(W\) remains scalar-only. |
| §0 no source failure, dimension-dependent constant, or route-level obstruction | **unchanged-VALID**. |
| §1.1 \(A_1\): domain, guard, product, \(J\), every exact-unit axiom including \(\|J\|=1\), \(\varepsilon_r=C_{\rm rect}\varepsilon_X\), and both closeness estimates | **unchanged-VALID**; the clause is byte-identical to v4. |
| §1.2 \(A_2\): domains, guard, unique graph value, \(f_V\), chart point/cover, and all three estimates | **unchanged-VALID**; the clause is byte-identical to v4. |
| §1.3 \(A_3\): tangent image, global \(C^1\) trivialization/distortion, scalar equivariance, and \(\omega_U(iU)=iJ\) were present | **unchanged-VALID** conclusions. |
| §1.3 \(A_3\): “the family” was unbound and graph equation, chart membership, and three graph estimates were added conclusions | **CLEARED-BY** universal quantification of a \(C^1\) family identified only by the unique zero equation; chart membership and the three graph estimates are absent from the consequent. |
| §1.4 \(A_4\): finite-dimensional domain, polar guard, \(C^1\) diffeomorphism/inverse, three inverse identities, openness, and exact sandwich | **unchanged-VALID**; the clause is byte-identical to v4. |
| §1.5 \(A_5\): all seven group identities/estimates were present | **unchanged-VALID** conclusions. |
| §1.5 \(A_5\): finite-dimensional domain insertion | **CLEARED-BY** the literal every-exact-unit domain. |
| §1.5 \(A_5\): inverse and map conclusions changed to an if-conditional | **CLEARED-BY** the affirmative unique-inverse binder and affirmative global \(C^1\) definitions of \(\mu,\sigma\). |
| §1.6 \(A_6\): \(q\)-range, all guards, invertibility, approximate-unitary membership, joint continuity, endpoints, and equivariance were present | **unchanged-VALID** conclusions. |
| §1.6 \(A_6\): finite-dimensional domain insertion | **CLEARED-BY** the literal every-exact-unit domain and local unique-first-inverse-component binder. |
| §1.7 \(A_7\): five guards, chart retention, and the \(-2I\) derivative bound were present | **unchanged-VALID** conclusions. |
| §1.7 \(A_7\): finite-dimensional domain insertion | **CLEARED-BY** the literal every-exact-unit domain. |
| §1.7 \(A_7\): conditional polar inverse and graph package | **CLEARED-BY** definite unique-inverse and unique-\(C^1\)-graph binders plus local definitions of \(\chi_s,\sigma\); no graph estimate is repeated. |
| §1.8 \(R\): four finite-minimum equations and all ten scalar inequalities | **unchanged-VALID**; the clause is byte-identical to v4. |

### 10.2 Meta-language, self-containment, and budget

| audit-v4 finding | disposition |
|---|---|
| §2.1 forbidden meta-language is absent and the scalar assertions are explicit | **unchanged-VALID**; the repairs add no contract-text operation, imported-conclusion placeholder, or mathematical row reference. |
| §2.2 \(W\) has fourteen scalar fields and no maps | **unchanged-VALID**; neither definition-content cell changes. |
| §2.2 \(g_U\) unbound in \(A_3\) | **CLEARED-BY** the universal family binder and minimal unique-zero characterization. |
| §2.2 polar inverse scoped only in a different conjunct or conditional antecedent | **CLEARED-BY** independent local unique-inverse binders in \(A_5,A_6,A_7\). |
| §2.2 \(\chi_s\) nested as an if-clause | **CLEARED-BY** the direct local definition of \(\chi_s\) before the two derivative conclusions. |
| §3 v4's \(11/3\) list fused producer applications with transports without evidence | **CLEARED-BY** the declared convention in §2 and the seven helper rows, each with an explicit \(4/2\) producer-instantiation/monotonicity tree. |
| §3 honest unfactored projection is \(18/4\) | **CLEARED-BY** factoring, not denial: §2 records \(18/4\) as the rejected unfactored tree and gives row 13 the factored \(11/3\) tree. |
| §3 helpers must receive the same \(W\), not unrelated existential tuples | **CLEARED-BY** every helper is universal over a receiving `def-stage1-polar-witness-data` tuple \(W\); row 13 selects one \(W\) after collecting the seven base packages. |
| §3 cap may not be raised | **unchanged-VALID**; every helper is \(4/2\) and row 13 is \(11/3\). |

### 10.3 Downstream rows, carry-forward, and source integrity

| audit-v4 finding | disposition |
|---|---|
| §4 uniform inversion isolation `defs` | **unchanged-VALID**: `def-epsilon-cstar-algebra`; `def-approximate-unitary-space`. |
| §4 quotient manifold package `defs` | **unchanged-VALID**: `def-approximate-unitary-space`; `def-epsilon-cstar-algebra`. |
| §4 quotient finite-CW `defs` | **unchanged-VALID**: `def-approximate-unitary-space`; `def-epsilon-cstar-algebra`. |
| §4 quotient left inversion `defs` | **unchanged-VALID**: `def-approximate-unitary-space`; `def-h-space-left-inversion`; `def-epsilon-cstar-algebra`. |
| §4 quotient inversion/index data `defs` | **unchanged-VALID**: `def-approximate-unitary-space`; `def-lefschetz-fixed-point-data`; `def-epsilon-cstar-algebra`. |
| §4 maximal-simplex placement `defs` | **unchanged-VALID**: `none`. |
| §4 all six downstream contracts, dependency lists, and budgets | **unchanged-VALID**; no downstream row is altered. |
| §5 rows 1–12 protected carry-forward | **unchanged-VALID** and byte-identical. |
| §5 downstream contracts/deps/budgets after normalizing the v4 `defs` column | **unchanged-VALID** and byte-identical. |
| §5 normalized downstream SHA256 | **unchanged-VALID**: `93d0858b55c118cd3be0e12f876a373aebf0259f9300ae419e870b8427ebbf0d`. |
| §5 obligation ledger and ten-id list | **unchanged-VALID** and byte-identical. |
| §5 dimension-freeness audit | **unchanged-VALID** and byte-identical. |
| §5 definition provisioning | **unchanged-VALID** and byte-identical. |
| §5 serial dependency order | **unchanged-VALID** as a dependency relation; only the seven helper rows are inserted before the ledger in §9's topological order. |
| §5 local-source/hash section | **unchanged-VALID** and byte-identical. |
| §5 Kitaev SHA256 | **unchanged-VALID**: `e7eb512a2ec2438d139c581fe48c017a6ffdc87c37f6fa7492159b757a9a9acb`. |
| §5 Lee SHA256 | **unchanged-VALID**: `324b7d8b1f70d40eb7608919e3c9cef93628215fa9e9f0816cb4c9549f058b3c`. |
| §5 Munkres SHA256 | **unchanged-VALID**: `9fcbbac92a09926498c1caba8fafa61b1a3568033485b3977edc523cc0459e5d`. |
| §5 pinned Kitaev, Lee, and Munkres interfaces and no **NOT IN LOCAL REFS** escalation | **unchanged-VALID**; §1 is byte-identical to v4. |

### 10.4 Every required correction in audit §6

| audit-v4 required correction | disposition |
|---|---|
| §6(1) restore literal producer domains in group, path, derivative | **CLEARED-BY** \(A_5,A_6,A_7\) and their transport helpers. |
| §6(2) replace unbound/conditional map prose by exact dependency binders, adding no guard or conclusion | **CLEARED-BY** \(A_3,A_5,A_6,A_7\); only unique defining properties identify imported maps. |
| §6(3) preserve every correct estimate and identity in \(A_1,A_2,A_4,R\) and the repaired clauses | **CLEARED-BY** byte-stable valid clauses and formula-for-formula retained producer conclusions. |
| §6(4) re-project and factor if over \(12/3\) | **CLEARED-BY** seven \(4/2\) helpers and the \(11/3\) common ledger under one declared convention. |
| §6(5) preserve downstream `defs`, carry-forward material, and design-only ceiling | **unchanged-VALID**; no registry or definition is mutated and no status is promoted. |
| Final instruction: do not land or seed v4; redesign row 13 and re-audit | **CLEARED-BY** this design-only v5 deliverable; a fresh hostile audit and user ratification remain mandatory. |

## 11. Final unblocking map

| formerly blocked interface | v5 disposition |
|---|---|
...
The correct next action is a **fresh hostile audit of this v5 design**. Nothing
