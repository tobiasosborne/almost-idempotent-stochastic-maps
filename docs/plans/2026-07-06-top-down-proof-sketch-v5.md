<!--
ROLE: the top-down, breadth-first FULL proof sketch of op-classical, VERSION 5 (session 10,
  round 2) — THE CANONICAL STRATEGIC MAP. Supersedes
  docs/plans/2026-07-06-top-down-proof-sketch-v4.md (kept intact — banked artifacts cite its
  line numbers) after waves W22 (trunk <2>5: Kernel ⇒ HLC reviewed + codified; op-hlc
  registered + finisher wiring), W23 (THE A-GAP CLOSED mod-review: lem-parametric-halo-collapse,
  T(a) = 5a/2 + 3, T(4) = 13, delta-window (17-12*sqrt2)/2; route (ii) moot), and W24 (step 3
  disintegration reviewed + codified). The g-bootstrap's remaining mathematical content is
  exactly ONE open step — step 4, the analytic anti-splitting residual, now posed with full
  precision.
...
PROVENANCE: v4 + session-10 round-2 banked waves (docs/waves/2026-07-06-W22-kernel-implies-hlc,
  2026-07-06-W23-a-gap, 2026-07-06-W24-step3-disintegration) + the codifications
  lem-kernel-implies-hlc, op-hlc, lem-parametric-halo-collapse, lem-genuine-disintegration.
-->

# Top-down proof sketch v5: op-classical (2026-07-06, session 10 round 2)
...
- **M1 — absorption forcing via THE G-BOOTSTRAP (USER-ADOPTED target of record; session-10
  ...
  **THE A-GAP — CLOSED mod-review (W23, round 2):** `lem-parametric-halo-collapse` [reviewed]
  ...
  P^t = P, so dynamic/mixing arguments are sterile BY DESIGN and are not used). **THE ONE
  REMAINING OPEN STEP of the bootstrap, now posed with full precision (post-W22/23/24):**
  let delta < (17-12*sqrt2)/2 and suppose H > 13*tau; then g := g^{(4)} = P*1_{G_4} is exactly
  P-harmonic with: every hidden top > 1/2 - delta [parametric collapse, reviewed]; every
  visible row in [-nu_w, 4*tau] [Lemma A, reviewed]; every row with g >= 1/2 disintegrating
  >= 1/2 - slack onto hidden vertices at depth > 4*tau [step 3, reviewed]. DERIVE A
  CONTRADICTION — anti-splitting in analytic (count-free) form. [OPEN — the kill zone; the
  residual is purely analytic (W20: empirically unreachable, zoo measurement retired).
  obs-deep-leakage RE-PRICED: the width-4 surface was derived WITHOUT it — whether step 4
  still consumes it is now a design question of the step-4 wave, not a standing blocking dep;
  aism-tq3 demoted from blocker to check-at-design.]
...
**Deciders (re-ranked 2026-07-06 session 10 round 2; aism-vmt/0b1/sg6/o1x/pu0 all DONE):**
(1) **THE STEP-4 WAVE** — the once-applied maximum principle on the precise width-4 surface
above (prove-or-refute pair + hostile verifier; wave design must decide whether
obs-deep-leakage is consumed); (2) trunk <2>6 re-audit (the staleness rule's next debtor);
(3) aism-88r extended — af elevation of the reviewed quartet (lem-visible-g-small,
lem-parametric-halo-collapse, lem-genuine-disintegration, lem-kernel-implies-hlc; all deps
af-validated for the middle two — USER DECISION); (4) aism-pld — poke-charge codification +
distinct-multi-class optimization (M2 fallback; still gated on realizing depth > 1*tau);
(5) aism-yxa re-aimed — D1 codification targeting the parametric form (B = 13 at width 4).
...
Route A (primary): 5. MIN-A via M1 = THE G-BOOTSTRAP [session-10-round-2 state: steps 1-3 ALL
stand at reviewed tier or better (step 1 realized; step 2 lem-visible-g-small; step 2' a-gap
CLOSED via lem-parametric-halo-collapse, T(4) = 13, delta-window (17-12*sqrt2)/2; step 3
lem-genuine-disintegration); **step 4 = THE ONE REMAINING OPEN STEP** — the once-applied
maximum principle on the precise width-4 surface (see M1 block); M2 poke-charge = fallback;
M3 subsumed into the g-frame]; 6. obs-deep-leakage [RE-PRICED: no longer a standing blocking
dep — the width-4 surface was derived without it; check-at-design of the step-4 wave,
aism-tq3]; 7. W-nonemptiness [genuine; SBD reset import aism-1qd is the candidate tool;
NOTE: consumed by conj-kernel itself — Kernel(i)]; 8. delta=0 endpoint [short]; 9. D1
codification re-aimed at the parametric form [bookkeeping, aism-yxa].
...
- The g-bootstrap step 4: the STATEMENT is now fully scoped (the width-4 surface, M1 block);
  the MECHANISM is the sole unscoped mathematics of Route A. Its wave design must also decide
  the obs-deep-leakage consumption question.
