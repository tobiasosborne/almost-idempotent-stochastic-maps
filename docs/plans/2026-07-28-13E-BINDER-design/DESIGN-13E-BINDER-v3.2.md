# DESIGN v3.2 — quantifier-closure transcription (mechanical)

Provenance: this file applies, VERBATIM and mechanically, the prescribed
repair of `AUDIT-13E-BINDER-v3.1.md` finding 1 to the v3.1 contract — the
two explicit universal quantifications the auditor spelled out ("for every
V in calU and every A^par in B_{2delta}^{icalH}(0), g_V(A^par) is the
unique ..." and the typed inverse identities). No other token of the v3.1
contract is changed; the orchestrator transcribed the auditor's wording
and authored nothing. Everything else of DESIGN-13E-BINDER-v3.md +
DESIGN-13E-BINDER-v3.1.md (deps, defs, external list, 12/18 cap, MINOR-5
row, delta scope) is unchanged.

## The contract (replaces v3.1 §1 line 6 only)
...
Diff against v3.1, exhaustively: (i) the graph predicate now opens
"such that, for every V in calU and every A^par in
B_{2delta}^{icalH}(0), g_V(A^par) is the unique A^perp ..."; (ii) the two
inverse identities are typed: "... = X for every X in S_delta and ... =
(U,H) for every (U,H) in calU x B_delta^{calH}(J)". Nothing else changed.
