# DESIGN-S1-ENDGAME-v4 — Stage-1 endgame repair round 3
...
## 0. Executive disposition of audit-v3 findings

This is a self-contained replacement for `DESIGN-S1-ENDGAME-v3.md`. It
preserves every surface that audit v3 accepted and repairs all five findings.

| finding | disposition |
|---|---|
| **F1** | **FIXED by R-G1 option (a), PARAMETERIZE.** B0a, B0i, B0s, and B0b are universal conditional rows on one explicitly quantified fourteen-field `W`. Each root spells the exact specialized (A_2),(A_4)--(A_7),(R) clauses it consumes; none says “supplied by,” and none depends on or applies the existential ledger. B1 is the unique instantiator: it applies `lem-stage1-polar-constant-ledger` exactly once, binds that `W`, and applies B0b to the same witness. This directly repairs the non-unique selection and second-application defect at `AUDIT-S1-ENDGAME-v3.md:38-66`; the data-only warning at `definitions/def-stage1-polar-witness-data.md:13-28` is respected. |
| **F2** | **FIXED.** Every B root first binds `a space breve-calU` and asserts `breve-calU=calU_e/U(1)`; it then explicitly says `set breve-e:=[J]` before the symbol is used. A contract-wide binder scan is recorded in section 8. This repairs `AUDIT-S1-ENDGAME-v3.md:68-90`. |
| **F3** | **FIXED by R-G3 option (ii), EXPLICIT AMBIENT BRIDGE.** Option (i) is unavailable under the root-only rule: `lem-stage1-uniform-inversion-isolation` concludes isolation for bare “the smooth sigma” without binding or defining that map (`argument/lemmas/lem-stage1-uniform-inversion-isolation.md:4`), exactly the synchronization warning at `AUDIT-S1-ENDGAME-v3.md:120-124`. B0i instead proves and exports, for its displayed `chi_s` and displayed `sigma(U)=u_delta(U^dagger)`, the universal clauses `calU intersect B_{r_iso}(sJ) subseteq chi_s(B_{r_iso}^{icalH}(0))` and `||A-B||<=||chi_s(A)-chi_s(B)||`. B0s combines those clauses with QIFT injectivity for `F_s-id`. Thus its ambient `r_bidx` is dimension-free and belongs to the same map; the missing bridge at `AUDIT-S1-ENDGAME-v3.md:92-130` is no longer asserted. |
| **F4** | **FIXED.** A2 consumes the finite positive-positive coproduct tail only from A1's root, its sole dependency. The skeleton and budget table say this explicitly, correcting `AUDIT-S1-ENDGAME-v3.md:134-143`. |
| **F5** | **FIXED.** A1 has a separate node that grades the abstract exterior algebra by the degrees of the homogeneous odd generators and proves that the generator-to-generator algebra isomorphism preserves degree. The registered 3C.4 external remains only “as an algebra”; the grade argument uses the local guide at `approximate_algebras.tex:1016`. This repairs `AUDIT-S1-ENDGAME-v3.md:145-160`. |

The design still has **13 rows**. Relative to v3 it adds no row, definition,
external id, or reference. B0i and B1 are repriced; all targets are at most
eleven and every hard cap is at most fifteen.
...
6. **(R), scalar data actually used:** the exact formulas for
...
mathematical content. B1 alone imports the ledger and binds one witness
satisfying it.
...
5. The fixed T0 `lem-stage1-uniform-inversion-isolation` remains in B1's
   user-fixed ten-item list but is not used to identify an anaphoric map.
   Same-map ambient isolation is supplied by B0b.

### Block C — one-use projection bridge and the three G-S1 producers

Audit-v3's accepted C0/C1 architecture (b) is unchanged.
...
B0a--B0b are proved as conditional rows before B1. B1 then selects the ledger
witness once and instantiates the already-proved conditional chain.
...
1. Fix the universally quantified `W` and proofs of its physically displayed
   (A_2),(A_4)--(A_6),(R) antecedents; derive one receiving threshold. Do not
   invoke the ledger.
2. Bind `(u_delta,h_delta)` as the unique inverse of the displayed
   `Pi_delta_*`.
3. Feed the same graph/atlas/inverse objects to the explicit smooth-operations
   root.
4. Obtain the exact formulas, `sigma(J)=J`, covariance, and near-adjoint
   estimate for this map.
5. Bind `breve-calU`, set `breve-e:=[J]`, define the two descended maps, and
   prove representative independence.
6. Attach smoothness through local quotient sections.
7. Use the displayed group/path clauses to prove the H-space laws and the
   left-inversion homotopy.
8. Assemble the conditional package, retaining the literal estimate.

### B0i. `lem-stage1-bound-quotient-local-index` — 10 nodes

1. Apply B0a to the already fixed `W`; bind its displayed objects once.
2. Apply the quotient-manifold root directly and record every manifold field.
...
1. Apply B0s to the same fixed `W`; no map, inverse, or radius is reselected.
2. Apply the manifold root directly and identify the canonical quotient.
...
### B1. `lem-stage1-extra-fixed-class` — 11 nodes

1. Apply `lem-stage1-polar-constant-ledger` **once**, existentially bind its
   `W`, and record the exact (A_2),(A_4)--(A_7),(R) conjuncts appearing in the
   B1 root. This is the only ledger application in the B chain.
2. Set `e_fix^r<=epsilon_*^r`, `C_fix>=C_grp`, and
   `r_bidx=r_iso`; apply conditional B0b to this same `W` and bind its
   complete package.
3. Assume `breve-e` is the only fixed class.
4. Place that singleton fixed set in a maximal simplex.
5. Apply Lefschetz-Hopf and the local index to get
   `Lambda(breve-sigma)=1`.
6. Apply A3 to this same H-space and left inversion.
7. Substitute the traces to obtain the sum of Betti numbers.
8. Use connected degree zero and positive-dimensional orientable top
   cohomology to make that sum at least two.
9. Contradict node 5 and choose `breve-U!=breve-e`.
10. Apply B0b's class-first phase clause, bind `U_0,c,a,U`, and combine
    fixedness with the literal near-adjoint estimate.
11. Use the same-map ambient isolation clauses to get both distance bounds and
    assemble.
...
2. Record exact-unit axioms, product/unit closeness, and the scale relation.
3. Apply C0 exactly once; do not apply B1.
...
| B0a | `W` is fixed before the input algebra; all operations are formula-defined and thresholds are finite minima of its universal fields. |
...
| B1 | The ledger is selected once before the algebraic/topological contradiction; only two nonzero cohomological degrees are needed. |
| C0 | The line-939 formula has a fixed number of terms and uses the proved universal ambient radius. |
| C1--C3 | Rectification, complement, and four basis-product estimates are fixed-term; `:458` excludes dependence on extra data. |
...
| Thresholds | M19-S1 takes one finite minimum after M04 and M01--M18 exist. |

Thus the M19-S1/M15 clause match accepted at
`AUDIT-S1-ENDGAME-v3.md:245-275` is unchanged. This design removes only the
G-S1 blocker.
...
- Every conditional B row binds `W` before its fields, the algebra before
...
- B0b binds `breve-V` before `U_0,c,a`; B1 binds `breve-U` before
  `U_0,c,a,U`.
- C0's existential projection is typed by its input exact-unit algebra; C1--C3
  bind their extended algebra, product, unit, projection pair, and map before
  use.
...
with `A3 -> B1` and the one T0 ledger edge entering B1.
...
are additional typed implementation providers required by R-G1(a).

## 9. Honest budgets and build granularity

| row | v3 target / cap | v4 target / rounds / cap | reason |
|---|---:|---:|---|
| A0 `hspace-coproduct-tail` | 10 / 15 | 10 / 4 / 15 | Unchanged. |
| A1 `exterior-cohomology` | 7 / 11 | 8 / 3 / 12 | Adds the explicit grade-preservation node required by F5. |
| A2 `associated-graded` | 9 / 14 | 9 / 3 / 14 | Same mathematics; consumes A1's root only. |
| A3 `trace` | 4 / 8 | 4 / 2 / 8 | Unchanged. |
| B0a `bound-quotient-left-inversion` | 8 / 12 | 8 / 3 / 12 | Parameterized hypothesis matching replaces the illegal selection. |
| B0i `bound-quotient-local-index` | 8 / 13 | 10 / 4 / 15 | Adds one affine-QIFT node and one graph-uniqueness ambient-capture node. |
| B0s `bound-inversion-isolation` | 7 / 12 | 7 / 3 / 12 | Coordinate injectivity now consumes the proved ambient bridge. |
| B0b `bound-quotient-index-data` | 5 / 9 | 5 / 2 / 9 | Remains conditional; direct manifold/CW and phase lift only. |
| B1 `extra-fixed-class` | 10 / 15 | 11 / 4 / 15 | Separates the unique ledger selection from application of conditional B0b. |
| C0 `fixed-unitary-projection-bridge` | 8 / 12 | 8 / 3 / 12 | Unchanged. |
| C1 `rectified-nontrivial-projection` | 6 / 10 | 6 / 3 / 10 | Unchanged. |
| C2 `original-complementary-pair` | 6 / 10 | 6 / 2 / 10 | Unchanged. |
| C3 `fresh-two-point-inclusion` | 9 / 14 | 9 / 3 / 14 | Unchanged. |

The total target is **98 nodes across 13 rows**. Maximum target is eleven;
maximum hard cap is fifteen. B0i+B0s+B0b targets 22 nodes, honestly above the
validated quotient-index benchmark (12) plus isolation benchmark (7), because
the universal ambient-inradius bridge is now a separately visible two-node
obligation inside B0i. No new row is needed because B0i remains below the
approximately-twelve target rule.
...
| B0a | Is `W` still a disguised selection? | Universal typed binder plus physical antecedents; no ledger edge. |
| B0i | Are (A_7), the charts, and `sigma` literally the same objects? | Same `W`, unique `Pi` inverse, unique graph functions, and repeated formulas. Stop on any second inverse or map. |
...
| B0b | Is finite CW conditional, and is the class bound before its lift? | Direct manifold antecedent; class-first binder. |
| B1 | Is the ledger applied twice or is a different `W` passed? | One ledger node, followed by one application of conditional B0b to that bound witness. |
...
| C1 | Is B1 selected again? | C1 depends only on C0 plus the rectification ledger. |
...
User ratification is required for the same 13 contracts as v3. There is **no
new helper id, external id, or definition shard** relative to v3. The material
contract changes are:

1. B0a--B0b become conditional on one explicitly quantified, physically
   specified ledger witness;
2. B0i gains the universal ambient-chart inclusion and lower-Lipschitz clauses;
3. B1 becomes the unique ledger instantiator;
4. every B row binds `breve-calU` and sets `breve-e`; and
5. A1 explicitly prices grading.

A fresh hostile audit should attack the five repaired interfaces first, then
re-run the already-clean source, status, M19, and hand-off scans. If all rows
