...
> *Where's the LIVE PROOF STRATEGY?* → **`docs/plans/CURRENT.md`** (generated by
> `scripts/gen-current-pointer.py`; always points at the lexicographically-latest
> `docs/plans/*top-down-proof-sketch*` file) — the MOST DYNAMIC artifact in this repo. **Keeping it reconciled with
...
> (Rule 9). Supersede by dated file, then re-run `python3 scripts/gen-current-pointer.py`;
> old versions stay intact for line citations.** *Notation?* →
...
2. **Provers = fresh codex; verifiers = *separate* fresh codex; roles never mix.**
   ...
   **Model/effort policy (user directive 2026-07-09; amended 2026-07-13): codex = `gpt-5.6-sol`; effort is
   CAPPED at `xhigh` — `ultra` is unstable and spawns subagents indiscriminately, never use it. `xhigh` for
   truly creative/demanding jobs, lower (`high` or below) for lower-priority routine jobs.**
   `af-orchestrate.py` encodes this as `--tier creative` (prover `xhigh` / verifier `xhigh`, default) vs
   `--tier routine` (`high`/`high`), with `--prover-effort`/`--verifier-effort` overrides (also capped).
