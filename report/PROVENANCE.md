...
| lem:stage1-smooth-unitary-operations | ARG-LEM-STAGE1-SMOOTH-UNITARY-OPERATIONS AF-LEM-STAGE1-SMOOTH-UNITARY-OPERATIONS | registry contract and proof export node 1 | O | Contract quoted verbatim; status `proved`, `af: validated`; workspace `proofs/lem-stage1-smooth-unitary-operations` (15 nodes); the two cross-sibling challenges were repaired in-run by local membership derivations, and the first-run PROVER-OVERREACH abort was spurious (orchestrator's own `.frontier/` writes, guard exempted) — all proof-level, no contract amendment. |
...
| lem:stage1-maurer-cartan-transport | ARG-LEM-STAGE1-MAURER-CARTAN-TRANSPORT AF-LEM-STAGE1-MAURER-CARTAN-TRANSPORT | registry contract and proof export node 1 | O | Contract quoted verbatim; status `proved`, `af: validated`; workspace `proofs/lem-stage1-maurer-cartan-transport` (13 nodes; the two challenges — the conditional-only identification with the distinguished graph family — were repaired in-run and are proof-level, not contract amendments). |
...
| lem:stage1-polar-path-transport | ARG-LEM-STAGE1-POLAR-PATH-TRANSPORT AF-LEM-STAGE1-POLAR-PATH-TRANSPORT | registry contract and proof export node 1 | O | Contract quoted verbatim; status `proved`, `af: validated`; workspace `proofs/lem-stage1-polar-path-transport` (9 nodes, first-pass, zero challenges). |
