...
| lem:stage1-inversion-derivative-control | ARG-LEM-STAGE1-INVERSION-DERIVATIVE-CONTROL AF-LEM-STAGE1-INVERSION-DERIVATIVE-CONTROL | registry contract and proof export node 1 | O | Contract quoted verbatim; status `proved`, `af: validated` — RE-VALIDATED 2026-07-29 (W98 queue row 4, clean re-seed on the typed explicit-binder spine; 10/10 nodes, taint clean) after the 2026-07-28 retraction of the 2026-07-27 tree (adjudicated synchronization defect; `docs/LEARNINGS.md`); still typeset as a conjecture env pending the batched shard-48 restoration (a fresh prose account of the new tree); the sign variable `s` is renamed to varsigma, a notation change only. |
