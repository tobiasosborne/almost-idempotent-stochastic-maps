...
By the meaning of matrix amplification, $T_n\bigl([Z_{ij}]\bigr)=[T(Z_{ij})]$ acts
...
and the level-one lower-modulus hypothesis is at least $\tfrac14$,
...
off-diagonal statement \texttt{lem-hcb3-offdiagonal-inverse}
(\S\ref{sec:hcb3-offdiagonal-inverse}), the hcb4 canonical tier
(\S\S\ref{sec:hcb4-canonical-gram}--\ref{sec:hcb4-canonical-inverse}) and the parent
\texttt{conj-hcb} (\S\ref{sec:hcb}) are now all \texttt{proved} with
\texttt{af: validated}. Nothing here bears on the status of \texttt{op-classical}, which is now discharged at the af-validated rung via Route F (\S\ref{sec:status-outlook}); sharpness is now carried at T0 by \texttt{cor-classical-sharpness}.
