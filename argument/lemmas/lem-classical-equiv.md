---
id: lem-classical-equiv
kind: lemma
contract: The signed-idempotent and stochastic-idempotent formulations of classical stability are equivalent up to universal constants: Q row-stochastic with ||Q^2-Q|| <= eta gives P=theta(2Q-1) signed affine retraction with ||P-Q|| <= C eta and neg mass delta <= C eta, and conversely row-normalising p_i^+ gives Q with ||P-Q|| <= 2 delta, ||Q^2-Q|| <= 6 delta+4 delta^2.
defs: def-stochastic; def-signed-idempotent; def-negative-mass; def-almost-idempotent
deps: 
status: proved
af: validated
provenance: docs/ingest (classical-portfolio; contracts verbatim from ../almost-idempotent-positive-maps/argument/lemmas or report/kernel-conjecture.tex)
owner: A
workspace: proofs/lem-classical-equiv
---

The signed<->stochastic bridge. **af-VALIDATED IN-REPO 2026-07-02** (the first rigorous result here):
29-node adversarial tree, root `validated`, taint all-clean; fresh codex prover/verifiers per node, Claude
orchestrated only (§6). Ledger: `proofs/lem-classical-equiv/ledger/`; human-readable export:
`proofs/lem-classical-equiv/export.md`. (Was also af-validated upstream; that run is superseded by this
in-repo validation.)
