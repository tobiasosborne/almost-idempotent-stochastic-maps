...
**Status.** af-VALIDATED 2026-07-28 (107th rigorous result; W95 run 2):
root validated, 13/13 live nodes, taint clean (22 nodes, 9 archived from
the in-run repair of one e-binding challenge; tier routine: prover high /
verifiers high). Export in the workspace; oracle
`af-lem-stage1-inversion-derivative-transport` + `fr verify` PASS.
Originally transcribed VERBATIM from the audited `DESIGN-S1-POLAR-v6.md`
§3 row 13g (final verdict LAND; the v5-audit u_delta/g_{sJ}-outside-domain
defect is CLEARED-BY the finite-dimensional insertion matched to base
row 8 and rows 2/4).
