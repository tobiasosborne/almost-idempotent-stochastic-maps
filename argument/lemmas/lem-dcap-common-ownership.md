...
deps: lem-l5-positive-flow-foldback; lem-dcap-kernel-bulk-census
status: proved
af: none
provenance: W65 wave (docs/waves/2026-07-13-W65-artifacts/): codex strategist-prover (gpt-5.6-sol, xhigh) DCAP-ATTACK-W65.md §1.5; fresh routine prover (gpt-5.6-sol, high) APPENDIX-W65-dcap-proofs.md; fresh hostile batched codex verifier (gpt-5.6-sol, xhigh), verdict VERDICT-W65-DCAP-BATCH.md line B3: VALID. Reviewer != author.
owner: B
---

**Role (W65 D-cap tree, B3 — common ownership).** Turns the routed census cell into constant top-owned mass and supplies the X coupling or intrinsic I/D vertex set used downstream.
...
**Honest scope.** Exactly one common nonnegative statistic is folded in the realized case; no actor-dependent demands are summed, I/D become intrinsic by diagonal Diracness, X retains barycentric provenance rather than transition mass, and the overlay need not use the original \(\eta_D^*\) carriers. Fallback: retain the sharper pre-error floors \(c_m/384\) in X and \(c_m/768\) in I/D.
