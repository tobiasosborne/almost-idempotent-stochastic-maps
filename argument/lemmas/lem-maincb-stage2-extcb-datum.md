...
**Status.** af-VALIDATED in-repo (2026-07-30, run 4, NON-VACUOUS):
12-node tree, root `validated`, taint clean 12/12
(`proofs/lem-maincb-stage2-extcb-datum/export.md`; oracle pass; tier
routine; parallel-af worktree run af-m13). Proved under the
USER-RATIFIED amended def-maincb-partition-state ('one current
nonempty subset U of J') with SATISFIABLE hypotheses — the earlier
vacuous validation (root closed by hypothesis contradiction under the
pre-amendment class-union field) is superseded; its ledgers are in git
history. The fresh verifier confirmed every EXT-CB datum clause
assembled from the T0 providers with no forbidden assumptions. MAIN
campaign row M13.
