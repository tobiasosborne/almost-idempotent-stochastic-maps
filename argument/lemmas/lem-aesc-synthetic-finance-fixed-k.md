...
deps: lem-hx-transverse-moment-identity; lem-dcap-five-way-completion-split
status: proved
af: none
provenance: W67 wave (docs/waves/2026-07-14-W67-artifacts/): codex strategist (gpt-5.6-sol, xhigh) AESC-ATTACK-W67.md §1.2; fresh routine prover (gpt-5.6-sol, high) APPENDIX-W67-aesc-proofs.md; fresh hostile batched codex verifier (gpt-5.6-sol, xhigh), verdict VERDICT-W67-AESC-BATCH.md line SF-K: VALID. Reviewer != author.
owner: B
---

**Role (W67 A-esc routine, SF-K — fixed-\(K\) synthetic-finance fallback).** Records the quantitative tail floor for a fixed larger hull-distance window.

**Mechanism (one line).** The synthetic-finance sign split gives \(\operatorname{Tail}_1(u)\ge(2\tau/15)(1-(3K+19)\tau/2)\), which is at least \(\tau/15\) when \(\tau\le1/(3K+19)\).

**Honest scope.** This is a pointwise node on the A-esc window; it does NOT prove the creative residual HES ([[conj-w67-aesc-hull-exterior-separator-synchronization]]) or DTR ([[conj-w67-aesc-diffuse-tail-ray-conversion]]), and no leaf exclusion or (EC) bound is claimed.
