...
PASS, 7-node tree all validated/clean, fresh codex provers with separate fresh hostile
verifiers per node, cap 21 held with wide margin (the three-helper factoring absorbed the
packet/estimate work as designed); external oracle `af-lem-routef-k-ledger` + `fr verify`
PASS. Mechanical reflection of the codex ledger. The strengthened statement was proved as
a NEW obligation; nothing was inherited from the superseded W74F paper ledger.
