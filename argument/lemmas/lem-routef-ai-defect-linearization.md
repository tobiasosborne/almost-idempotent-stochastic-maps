...
kind: lemma
contract: Approximate-algebra defect linearization: there are universal C_A < infinity and eta_A > 0 such that, for 0 <= eta <= eta_A, the image of tilde-Phi is an extended epsilon_AI(eta)-C*-algebra with epsilon_AI(eta) <= C_A*eta, where C_A=20+(211/8)*C_theta.
