...
**Dep wiring (per DESIGN-GAP-EA §2.3).** Depends on the GENERAL
`lem-extcb-exact-target-correction` (af-validated, T0 82) — an arbitrary
finite-dimensional source algebra enters, so the M_r-only bridge
(`lem-extcb-exact-target-approximation`) is deliberately NOT the dep.
