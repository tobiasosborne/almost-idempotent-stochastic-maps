...
**af-VALIDATED 2026-07-29 (W98, elevation queue row 3).** First-pass run
under the binding build-granularity discipline above (tier routine, fresh
codex verifier per node): root validated, 12/12 live nodes (EXACTLY the
design target; hard cap 18), taint clean, 6 rounds, ZERO challenges.
Export in the workspace; oracle
`af-lem-stage1-explicit-smooth-unitary-operations` + `fr verify` PASS.
