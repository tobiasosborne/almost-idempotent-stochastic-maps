...
deps: lem-ihorn-cotop-sl1a-package
status: proved
af: none
provenance: W65 wave (docs/waves/2026-07-13-W65-artifacts/): codex strategist-prover (gpt-5.6-sol, xhigh) DCAP-ATTACK-W65.md §1.3; fresh routine prover (gpt-5.6-sol, high) APPENDIX-W65-dcap-proofs.md; fresh hostile batched codex verifier (gpt-5.6-sol, xhigh), verdict VERDICT-W65-DCAP-BATCH.md line B1: VALID. Reviewer != author.
owner: B
---

**Role (W65 D-cap tree, B1 — score-bulk transfer).** Converts the fixed D-cap certificate and co-top web into a score-good root set of constant normalized and top-owned mass before the X/I/D census.

**Mechanism (one line).** The two admissible score components have \(\lambda_A\)-mean below \(6\tau/7\), so Markov at \(12\tau/13\), followed by \(a_A=S(1-\theta)\lambda_A\), gives the two strict mass floors.

**Honest scope.** The normalized \(\lambda_A\) is not confused with the top-owned measure \(a_A\); score equality belongs to \(F\), and the original \((\phi,h)\) is retained without a cover, averaging, Jensen step, or new certificate. Fallback: retain \(\lambda_A\{s\le L\tau\}>1-6/(7L)\) for every \(L>6/7\).
