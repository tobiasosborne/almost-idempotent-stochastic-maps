...
**Status.** af-VALIDATED 2026-07-27 (89th rigorous result): root validated,
15/15 live nodes epistemic=validated (3 archived), taint clean 18/18 (fresh
codex prover, separate fresh codex verifiers, routine tier, single run).
Export in the workspace; oracle `af-lem-stage1-unitary-graph-control` +
`fr verify` PASS. Landed as a `stated` candidate transcribed VERBATIM from
the audited `DESIGN-S1-POLAR-v6.md` §3 row 2 (design budget 8/3).
