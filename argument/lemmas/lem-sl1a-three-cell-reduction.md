...
kind: lemma
contract: SL1a three-cell reduction: if conj-sl1a-off-diagonal-cell (constant delta_X), conj-sl1a-intersection-diagonal-cell (delta_I), and conj-sl1a-deep-diagonal-cell (delta_D) all hold — the three diagonal cells of the selected-corner mass M_X/M_I/M_D of def-selected-corner — then no exact signed idempotent P with 0 < delta(P) <= delta_0 = min(2^(-16),delta_D,delta_I,delta_X), nonempty visible set W, and hidden top vertex v of height H > 16sqrt(delta(P)) admits a probability measure lambda on row points supported on {x:||p_x-p_v||_1 >= 4sqrt(delta(P)) and dist_1(p_x,conv{p_w:w in W}) > H-4sqrt(delta(P))}, with barycenter b satisfying ||b-p_v||_1 <= 11sqrt(delta(P))/5 and integral a(p_x) dlambda(x) <= 4sqrt(delta(P))/13 for every admissible exposer a at v.
defs: def-signed-idempotent; def-negative-mass; def-visible-set; def-exposed; def-height; def-selected-corner; def-actor-hull; def-top-support-functional; def-co-top
deps: conj-sl1a-deep-diagonal-cell; conj-sl1a-intersection-diagonal-cell; conj-sl1a-off-diagonal-cell; lem-top-deficit-price; lem-harmonic-affine-bridge; lem-mass-split; lem-positive-exposedness-margin; lem-always-tight-dual-support
status: proved
af: none
...
owner: B
---
**Contract rewritten 2026-07-10 (notation -> defs; aism-0mm); the pre-rewrite contract is preserved verbatim below.** The inlined selected-corner machinery (phi, exposer h, corner row f, kernel xi, Gamma_f, C_f, B_F/B_N, F*(u)/T/O/K_T/K_O, M_X/M_I/M_D) moved into [[def-selected-corner]]; the premise is now stated as its three deps [[conj-sl1a-off-diagonal-cell]], [[conj-sl1a-intersection-diagonal-cell]], [[conj-sl1a-deep-diagonal-cell]] (whose own contracts carry the exact M-threshold conditions), and the CONCLUSION (which matches [[conj-straddling-web-exclusion]] verbatim — the E2 DAG edge) is byte-unchanged. The MATHEMATICAL CONTENT is identical.

> PRE-REWRITE CONTRACT (verbatim): If there exist universal delta_X, delta_I, delta_D in (0,2^(-16)] such that, writing tau = sqrt(delta(P)), D = 2+4delta(P), and C_W = conv{p_w:w in W}, a selected-corner configuration means a finite exact signed idempotent P with 0 < delta(P) <= 2^(-16), nonempty visible set W, hidden top vertex v of height H > 16tau, an affine phi with phi(p_v) = H, phi <= 0 on C_W, and |phi(a)-phi(b)| <= ||a-b||_1, an admissible exposer h at v, a row point f with ||p_f-p_v||_1 >= 4tau, dist_1(p_f,C_W) > H-4tau, and 2(H-phi(p_f))/D+h(p_f) <= 12tau/13, a probability kernel xi_x(u) from row points to geometrically distinct row vertices, constant on clone fibers, with p_x = sum_u xi_x(u)p_u and Dirac at vertex points, P_fx^+ = sum_{j:p_j=p_x} max(P_fj,0), Gamma_f(x,u) = P_fx^+ xi_x(u), C_f = {(x,u):H-phi(p_x) < 4tau, h(p_x) < 4tau, H-phi(p_u) < 4tau, h(p_u) < 4tau}, B_F = C_f intersect {(x,u):||p_u-p_v||_1 >= 4tau}, B_N = C_f intersect {(x,u):||p_u-p_v||_1 < 4tau}, and some B in {B_F,B_N} with Gamma_f(B) >= 1/4, and, for every hidden row vertex u, F^*(u) = {(g,t^*(u)):g is affine, g(p_u) = 0, 0 <= g(p_i) <= 1 for every row i, and g(p_r) >= t^*(u) whenever ||p_r-p_u||_1 >= 4tau}, T(u) = {r:||p_r-p_u||_1 >= 4tau and g(p_r) = t^*(u) for every (g,t^*(u)) in F^*(u)}, O(u) = {i:g(p_i) = 1 for every (g,t^*(u)) in F^*(u)}, K_T(u) = conv{p_r-p_u:r in T(u)}, K_O(u) = t^*(u)conv{p_i-p_u:i in O(u)}, M_X(B) = Gamma_f{(x,u) in B:p_x != p_u}, M_I(B) = Gamma_f{(x,u) in B:p_x = p_u and K_T(u) intersect K_O(u) != empty}, and M_D(B) = Gamma_f{(x,u) in B:p_x = p_u and K_T(u) intersect K_O(u) = empty}, no selected-corner configuration with delta(P) <= delta_X has M_X(B) > 1/8, no selected-corner configuration with delta(P) <= delta_I has M_X(B) <= 1/8 and M_I(B) >= 1/16, and no selected-corner configuration with delta(P) <= delta_D has M_X(B) <= 1/8, M_I(B) < 1/16, and M_D(B) > 1/16, then no exact signed idempotent P with 0 < delta(P) <= delta_0 = min(2^(-16),delta_D,delta_I,delta_X), nonempty visible set W, and hidden top vertex v of height H > 16sqrt(delta(P)) admits a probability measure lambda on row points supported on {x:||p_x-p_v||_1 >= 4sqrt(delta(P)) and dist_1(p_x,conv{p_w:w in W}) > H-4sqrt(delta(P))}, with barycenter b satisfying ||b-p_v||_1 <= 11sqrt(delta(P))/5 and integral a(p_x) dlambda(x) <= 4sqrt(delta(P))/13 for every admissible exposer a at v.
...
Assume, only as conditional hypotheses, that `conj-sl1a-off-diagonal-cell` supplies \(\delta_X\in(0,2^{-16}]\) and forbids selected-corner data with \(M_X(B)>1/8\), that `conj-sl1a-intersection-diagonal-cell` supplies \(\delta_I\in(0,2^{-16}]\) and forbids selected-corner data with \(M_X(B)\le1/8\) and \(M_I(B)\ge1/16\), and that `conj-sl1a-deep-diagonal-cell` supplies \(\delta_D\in(0,2^{-16}]\) and forbids selected-corner data with \(M_X(B)\le1/8\), \(M_I(B)<1/16\), and \(M_D(B)>1/16\), in each case whenever \(\delta\) does not exceed that conjecture's ceiling.
...
The three conjectures above are open conditional premises; none is treated as established.  Fix their ceilings first and define \(\delta_0\) as in the statement.  Suppose for contradiction that SL1a fails at this ceiling.  Push the row-index measure forward under \(i\mapsto p_i\) to a measure on geometrically distinct row points, retaining the notation \(\lambda\); total mass, the barycenter, all affine-exposer means, and the support inequalities are unchanged.  Thus there are an exact signed idempotent \(P\), a hidden top vertex \(v\), and a probability measure \(\lambda\) on row points with
\[
...
Every pair in \(B\) is either off diagonal or diagonal.  If \(M_X(B)>1/8\), all hypotheses of the conditionally assumed off-diagonal cell exclusion hold, contradicting it because \(\delta\le\delta_X\).  Hence suppose \(M_X(B)\le1/8\), which is the guard for both diagonal cells.  The diagonal mass is then at least
\[
 \Gamma_f(B)-M_X(B)\ge\frac14-\frac18=\frac18. \tag{7}
\]
Every diagonal carrier has exactly one of types I and D.  If \(M_I(B)\ge1/16\), the conditionally assumed intersection-diagonal exclusion, including its guard \(M_X(B)\le1/8\), is contradicted because \(\delta\le\delta_I\).  Otherwise \(M_I(B)<1/16\), which together with \(M_X(B)\le1/8\) gives the two guards for the deep diagonal cell, and (7) forces
\[
 M_D(B)>\frac18-\frac1{16}=\frac1{16},
\]
contradicting the conditionally assumed deep disjoint-diagonal exclusion because \(\delta\le\delta_D\).  These three guarded cases are pairwise disjoint and exhaust the residual: \(M_X(B)>1/8\), or \(M_X(B)\le1/8\) and \(M_I(B)\ge1/16\), or \(M_X(B)\le1/8\) and \(M_I(B)<1/16\), in which last case \(M_D(B)>1/16\) follows from (7).  The strict and weak inequalities assign every boundary: radial distance equality and radial mass equality belong to the far choice, off-diagonal equality \(M_X(B)=1/8\) belongs to the diagonal continuation, and type-I mass equality \(M_I(B)=1/16\) belongs to type I.
...
\]
with \(\mu\) a probability row, direct multiplication gives \((P')^2=P'\) and \(P'\mathbf1=\mathbf1\), convexity of negative mass gives \(\delta(P')=\delta(P)\), and the new row \((\mu P,0)\) is a convex combination of the embedded old rows, so the row polytope and its geometrically distinct vertices are unchanged.  This alone does not preserve \(W\) or \(H\): the new nonvertex row can be \(\rho\)-far and add an exposedness constraint.  Consequently the old web, selected row, and coupled measure are asserted to lift only for extensions whose recomputed exposedness data are separately proved unchanged; the implication above does not use transient extension.
